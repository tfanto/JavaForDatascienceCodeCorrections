...
public class SentimentExampleIterator implements DataSetIterator {
	 ...
	 *            data.
	 */
	public SentimentExampleIterator(String dataDirectory, WordVectors wordVectors, int batchSize, int truncateLength,
	...
	private DataSet nextDataSet(int num) throws IOException {
		...
		for (int i = 0; i < reviews.size(); i++) {
			...
			for (int j = 0; j < tokens.size() && j < maxLength; j++) {
				...
				temp[1] = j;
				featuresMask.putScalar(temp, 1.0); // Word is present (not padding) for this example + time step -> 1.0
													// in features mask
			}
